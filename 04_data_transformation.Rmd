...
stroke %>% group_by(cat_sbp) %>% summarize(minsbp = min(sbp),
                                           maxsbp = max(sbp))
```
The label $]$ means including while $($ means above. So the label $(120, 130]$ means above 120 and including 130 

### Recoding variables 

We use this function to recode variables from the old levels to the new levels. Let us say that our objectives is to:

1. modify the levels for variable **cat_sbp** by changing the levels from $(-Inf,120]$ to $\leq120$ and 
2. $(120,130]$ to `121-130` and 
3. $(130, Inf]$ for $\geq 130$
...
```{r } 
stroke <- stroke %>%
  mutate(cat_sbp = recode(cat_sbp, "(-Inf,120]" = "less or 120",
                          "(120,130]" = "121 - 130",
                          "(130, Inf]" = "131 or higher"))
stroke %>% count(cat_sbp)
```
