...
$$\begin{aligned}
natural\ log\ of\ count\ outcome = numerical\ predictors + categorical\ predictors
\end{aligned}$$
...
## Poisson regression\index{Poisson regression} for Count
...
$$\begin{aligned}
ln(count\ outcome) = &\ intercept + coefficients \times numerical\ predictors \\
...
```{r}
# minus gender
pois_attack_all1 = glm(attack ~ res_inf + ghq12, data = asthma, family = "poisson")
...
```{r}
pois_attack_allx = glm(attack ~ res_inf * ghq12, data = asthma, family = "poisson")
...
```{r}
AIC(pois_attack1, pois_attack2, pois_attack3, pois_attack_all1, pois_attack_allx)
```
...
```{r}
x2 = sum((asthma$attack - pois_attack_all1$fitted)^2 / pois_attack_all1$fitted)
```
...
```{r}
qpois_attack_all1_summ = summary(glm(attack ~ res_inf + ghq12, data = asthma, 
                                     family = "quasipoisson"))
...
```{r}
tib_pois_attack = tidy(pois_attack_final, exponentiate = TRUE, conf.int = TRUE)
tib_pois_attack
```
...
# Fit the model
pois_attack_final_by6 = glm(attack ~ res_inf + ghq12_by6, data = asthma_new, family = "poisson")
```
...
```
$$\begin{aligned}
ln(attack) = & -0.34 + 0.43\times res\_inf + 0.05\times ghq12 -0.03\times res\_inf\times ghq12
\end{aligned}$$
...
$$\begin{aligned}
ln(attack) & = -0.63 + 1.02\times res\_inf + 0.07\times ghq12 -0.03\times res\_inf\times ghq12 \\
...
```{r}
pred = predict(pois_attack_final, list(res_inf = "yes", ghq12 = 8), type = "response")
round(pred, 1)
```
...
new_data$attack_pred = round(predict(pois_attack_final, new_data, type = "response"), 1)
new_data
```

## Poisson regression\index{Poisson regression} for Rate
...
```{r}
pois_case = glm(case ~ cigar_day + smoke_yrs, data = smoke, family = "poisson", 
                offset = log(person_yrs))
summary(pois_case)
```
...
```{r}
pois_casex = glm(case ~ cigar_day * smoke_yrs, data = smoke, family = "poisson", 
                 offset = log(person_yrs))
summary(pois_casex)
```
...
qpois_case_summ = summary(glm(case ~ cigar_day + smoke_yrs, data = smoke, 
                              family = "quasipoisson", offset = log(person_yrs)))
...
cbind(smoke[index,], case_pred = pois_case$fitted[index]) %>%
  mutate(rate = round(case/person_yrs, 4), rate_pred = round(case_pred/person_yrs, 4))
```
...
```
and use `tbl_regression()` to come up with a table for the results. Again, for interpretation, we exponentiate the coefficients to obtain the incidence rate ratio, _IRR_.
```{r, message = FALSE}
...
$$\begin{aligned}
ln(case) = &\ ln(person\_yrs) -11.32 + 0.06\times cigar\_day + 0.96\times smoke\_yrs(20-24) \\
& + 1.71\times smoke\_yrs(25-29) + 3.21\times smoke\_yrs(30-34) + 3.24\times smoke\_yrs(35-39) \\
& + 4.21\times smoke\_yrs(40-44) + 4.45\times smoke\_yrs(45-49) + 4.89\times smoke\_yrs(50-54) \\
& + 5.37\times smoke\_yrs(55-59)
\end{aligned}$$
...
```{r}
qpois_attack_all1 = glm(attack ~ ., data = asthma, family = "quasipoisson")
```
