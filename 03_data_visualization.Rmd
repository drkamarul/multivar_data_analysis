...
1.  **graphics**\index{graphics} : a base R package, which means it is loaded everytime we open R
2.  **ggplot2**\index{ggplot2} : a user-contributed package by RStudio
3.  **lattice**\index{lattice} : a user-contributed package


Except for **graphics**\index{graphics} package (a base R package), other packages need to downloaded and installed into your R library. A few examples more specific graphical packages include:

1.  **survminer::ggsurvlot** : The **survminer**\index{survminer} R package provides functions for facilitating survival analysis and visualization. 
2.  **sjPlot**\index{sjPlot} : Collection of plotting and table output functions for data visualization
...
To work with **ggplot2**\index{ggplot2}, remember to
...
The official website for ggplot2\index{ggplot2} is here <https://ggplot2.tidyverse.org/>. It has excellent resources. It states that:

*ggplot2\index{ggplot2} is a plotting system for R, based on the grammar of graphics\index{Grammar of graphics}, which tries to take the good parts of base and lattice\index{lattice} graphics and none of the bad parts. It takes care of many of the fiddly details that make plotting a hassle (like drawing legends) as well as providing a powerful model of graphics that makes it easy to produce complex multi-layered graphics.*
...
2.  outputs - plots 
3.  codes (R scripts `.R`, R markdown files `.Rmd`\index{R markdown})
...
3.  SPSS\index{SPSS} file (`.sav`)
4.  Stata\index{STATA} file (`.dta`)
5.  SAS\index{SAS} file 

Packages that read these data include **haven**\index{haven} and **rio**\index{rio} packages. Below are the functions to read SAS\index{SAS}, SPSS\index{SPSS} and Stata\index{STATA} file using the **haven**\index{haven} package. 
...
The **ggplot2**\index{ggplot2} package is one of the core member of **tidyverse**\index{tidyverse} metapackage (<https://www.tidyverse.org/>). So, if we load the **tidyverse**\index{tidyverse} package, it means we are also loading other packages under the **tidyverse**\index{tidyverse} metapackage: which include **dplyr**\index{dplyr}, **readr**\index{readr}, **ggplot2**\index{ggplot2}. 
...
We will also load the **here**\index{here} package. This is useful to point to the codes to a specific folder in the project space. We will see this in action later. 
...
If you run the code and you get this message *there is no package called tidyverse*\index{tidyverse}, you need to install the **tidyverse**\index{tidyverse} package on your R IDE. 

To install the package, type `install.package("tidyverse")`\index{tidyverse} in the Console. Once the installation is complete, type `library(tidyverse)`\index{tidyverse} to load the package.

Alternatively, you can use the GUI\index{GUI} to install the package:

![Packages pane](tidyverse.png)

Now, type the package you want to install. For example you want to install the **tidyverse**\index{tidyverse} package
...
a. the built-in dataset in the **gapminder**\index{gapminder} package. You can read more about *gapminder*\index{gapminder} from <https://www.gapminder.org/>. The gapminder\index{gapminder} website contains many useful datasets and show wonderful graphics. It is made popular by Dr Hans Rosling.
...
In **ggplot2**\index{ggplot2} which is a package under **tidyverse**\index{tidyverse} package, we may use the $+$ sign to connect the function. And in R, your codes can span multiple lines. This will increase the visibility of the codes.  
...
1. a scatterplot\index{Scatterplot}
2. the scatterplot\index{Scatterplot} shows the relationship between year and life expectancy. 
...
More resources about **ggplot2**\index{ggplot2} package is available here https://ggplot2.tidyverse.org/reference/ggplot.html 
...
*ggplot2*\index{ggplot2} will automatically assign a unique level of the aesthetic (here a unique color) to each unique value of the variable, a process known as scaling. *ggplot2*\index{ggplot2} will also add a legend that explains which levels correspond to which values. The plot suggests that with higher gdpPerCap, there is also longer lifeExp. 
...
Each `geom_X()` in ggplot2\index{ggplot2} indicates different visual objects.
...
We are using the `here()` function now. If you notice, we use a new package called **here**\index{here}. This is a very useful package that can help get or save files in the correct path (including folder) even when we are using different machines. If we are not clear about this, do you remember any circumstances when the drive name changes automatically (especially when using thumbdrive) in different computers. By using `here()` from the **here**\index{here} package, we will always get to the correct path or folder. 
