...
There are a number of good references to help readers understand binary logistic regression better. The references that we list below also contains workflow that will be useful for readers when modelling logistic regression\index{Logistic regression}. We highly recommend readers to read Applied Logistic Regression [@hosmer2013], Logistic Regression\index{Logistic regression}: Self Learning Text [@kleinbaum2010logistic] to strengthen your theory and concepts on logistic regression. And next you can refer to A Handbook of Statistical Analyses Using R [@R-HSAUR] which provides in greater details the R codes relevant to logistic regression models.
