...
In a binary logistic regression model, the dependent variable has two levels (categorical). When the outcome variable has more than two levels or categories, the analysis are modelled by multinomial logistic regression and, if the multiple categories are ordered, by ordinal logistic regression\index{Ordinal logistic regression} (for example, the proportional odds ordinal logistic model).
...
Variables sex, status, dm and stroke type are labelled variables which means that even though they are coded as numbers, the numbers represent the groups or categories or levels of the variables. They are categorical variables.
...
The independent variable can be numerical or a categorical variable. To estimate the log odds\index{Log odds} (the regression parameters, $\beta$) for the covariate Glasgow Coma Scale (GCS), we can write the logit model\index{Logit model} as: 
...
- It is unlikely that only one variable (gcs or stroke type) is related with stroke. For example, cardiovascular disease has many factors that affect the outcome. So, it makes more sense to consider adding another seemingly important independent variable in the model. 
...
- we can add other covariates to adjust for the confounding effects
...
Let us rename the table appropriately to combine the results from the log odds\index{Log odds} and the odds ratio\index{Odds ratio}.
...
It is not advisable to assess the importance of variables based on their p-values or the Wald statistics. The better way is to use the likelihood ratio to compare models and assess the importance of variables.  
...
To decide if an interaction\index{Interaction} term should stay in the model, we suggest you consider the biological and statistical significance. If you think the interaction\index{Interaction} justifies both reasons, then it is preferred that you keep the interaction\index{Interaction} term in the model. For example, for our model:
