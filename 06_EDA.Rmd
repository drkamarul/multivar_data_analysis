...
-   perform EDA using graphical methods
...
Read more on **patchwork** package to learn to arrange multiple plot. 
...
Line graphs are typically used for visualizing how one continuous variable on the y-axis changes concerning another continuous variable on the x-axis. This is very useful for longitudinal data. let us Load **gapminder** package\index{gapminder} so we can use the *gapminder* data.

```{r}
library(gapminder)
```

Line graphs can also be used with a discrete variable on the x-axis. This is appropriate when the variable is ordered (e.g., "small," "medium," "large"). We can examine the graphical relationship between variable life expectancy for the Asia continent and for Malaysia in comparison. To do so,  we

- specify the data. In this case gapminder data
- filter only continent == "Asia"
- choose variable life expectancy
- examine the trend using `geom_line()`
...
But you may use **gtsummary** package if you fancy it. We filter in `Asia` only. 
...
gapminder %>%
  ...
  modify_caption("**Characteristics for 1962, 1982 and 2002 for Asia** (N = {N})") 
```
...
These links contain excellent resources to help R users with exploratory data analysis:

- *ggplot2: Elegant Graphics for Data Analysis*\index{ggplot2} [webpage](https://ggplot2-book.org/arranging-plots.html) or its physical book [@chang2013r]. 
- **ggplot2**\index{ggplot2} [package](https://ggplot2.tidyverse.org/)
- **gapminder**\index{gapminder} [package](https://cran.r-project.org/web/packages/gapminder/README.html)
- **gtsummary** [webpage](https://www.danieldsjoberg.com/gtsummary/) [@gtsummary]
- *R for Data Science* [website](https://r4ds.had.co.nz/) or its physical book [@Wickham2017R]. 

## Summary

This chapters provides understanding to readers about exploratory data analysis (EDA) and skills for them to perform EDA using graphical methods and reporting descriptive statistics using tables. They also acquire basic skills to read a MS Excel data and then use three important packages to perform EDA (**dplyr**, **ggplot2** and **gtsummary** packages) on the data. Both **dplyr** and **ggplot2** packages can be loaded individually or by loading the **tidyverse** package. 
